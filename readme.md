...
### 4. User Input
- I wanted the user to have the option to return home or exit the app at any input point throughout the whole program. To do so, I needed to create input functions to replace the built-in method, to remove the necessity of checking at each input point, if the user typed \home or \exit. To keep the code DRY, this was the best approach.
- I created one input to handle strings only (no integers), and one for all types. This was part of my thinking as some responses would never require a number type in any form. In those situations, I used the relevant input that I created.
- The input function I created to check for numbers is below:
    ```python
    while True: # this will continously ask user for input until a break condition
      ...
      if user_input.isalpha(): # this is only True if the entire string is all alphabetical
        break
  return user_input
    ```
    For example, if the user is asked to enter a recognized language, of course no language has a number it in. So in that situation, I used the above code snippet. This also allowed for the user to return home or exit as well.
